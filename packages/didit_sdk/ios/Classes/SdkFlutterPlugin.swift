    ...
    private func presentVerification(result: @escaping FlutterResult, startAction: @escaping () -> Void) {
        guard self.hostingController == nil else {
            result([
                "type": "failed",
                "errorType": "unknown",
                "errorMessage": "Verification UI is already presenting."
            ])
            return
        }
        ...
        // Start verification in the completion block so it fires exactly once,
        // after the hosting controller is fully presented. Using onAppear caused
        // a double-launch on iOS because SwiftUI re-fires onAppear when the
        // Didit verification screen is dismissed and the bridge view reappears.
