...
class DetailLogmentAmentities extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (residenceModel.commodites.isEmpty) return const SliverToBoxAdapter();

    // Show max 6 items in the overview
    final displayCount = residenceModel.commodites.length.clamp(0, 6);
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    if (leftIdx < displayCount)
                      Expanded(
                        child: _AmenityItem(
                          icon: residenceModel.commodites[leftIdx].icon,
                          text: residenceModel.commodites[leftIdx].text,
                        ),
                      )
                    else
                    ...
                    if (rightIdx < displayCount)
                      Expanded(
                        child: _AmenityItem(
                          icon: residenceModel.commodites[rightIdx].icon,
                          text: residenceModel.commodites[rightIdx].text,
                        ),
                      )
                    else
                      const Expanded(child: SizedBox()),
